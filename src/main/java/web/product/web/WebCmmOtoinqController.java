 ...
 */
@Controller
public class WebCmmOtoinqController {
    ...
    @RequestMapping("/web/cmm/otpinqList.ajax")
    public String otpinqList(	@ModelAttribute("OTOINQVO") OTOINQVO otoinqVO,
    							@ModelAttribute("searchVO") OTOINQSVO otoinqSVO,
    		   					ModelMap model){


    	log.info("/web/cmm/otpinqList.ajax call "
    			+"[corpId:" + otoinqVO.getCorpId() + "]"
    			+"[Prdtnum:" + otoinqVO.getPrdtNum() + "]"
    			+"[CorpCd:" + otoinqVO.getCorpCd() + "]"
    			+"[CorpTel:" + otoinqVO.getCorpTel() + "]"
    			+"[PageIndex:" + otoinqVO.getPageIndex() + "]");
    	...
    	log.info("/web/cmm/otpinqList.ajax call 1");
		...
		log.info("/web/cmm/otpinqList.ajax call 2");
		...
		log.info("/web/cmm/otpinqList.ajax call 3");
		...
		log.info("/web/cmm/otpinqList.ajax call 4");
		...
		model.addAttribute("otoinqPaginationInfo", paginationInfo);

		//log.info("/web/cmm/otpinqList.ajax 호출>>>>>end:");

		log.info("/web/cmm/otpinqList.ajax call 5");

    	return "/web/cmm/otoinqList";
    }


    @RequestMapping("/web/cmm/otoinqInsert.ajax")
    ...
    @RequestMapping("/web/cmm/otoinqUpdate.ajax")
    public void otoinqUpdate(	@ModelAttribute("OTOINQVO") OTOINQVO otoinqVO,
    	...
    	if(EgovUserDetailsHelper.isAuthenticated()){
    		...
    		otoinqVO.setFrstRegIp(userIp);
    		otoinqVO.setLastModIp(userIp);
    	}else{
    		return;
    	}
    ...
    @RequestMapping("/web/cmm/otoinqDelete.ajax")
    ...
    @RequestMapping("/web/cmm/otoinqIntiUI.ajax")
    public ModelAndView IntiUI(@ModelAttribute("OTOINQVO") OTOINQVO otoinqVO,
								@ModelAttribute("searchVO") OTOINQSVO otoinqSVO,
    							HttpServletRequest request,
								HttpServletResponse response,
    		   					ModelMap model)  throws Exception{
    	...
    	log.info("/web/cmm/otoinqIntiUI.ajax call "
    			+"[corpId:" + otoinqVO.getCorpId() + "]"
    			+"[Prdtnum:" + otoinqVO.getPrdtNum() + "]"
    			+"[CorpCd:" + otoinqVO.getCorpCd() + "]"
    			+"[PageIndex:" + otoinqVO.getPageIndex() + "]");
    	...
    	log.info("/web/cmm/otoinqIntiUI.ajax call 1");

    	resultMap.put("Status", "success");

    	//숫자
    	resultMap.put("OtoCnt", ""+nCnt);
