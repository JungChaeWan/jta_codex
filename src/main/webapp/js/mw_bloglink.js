	...
	$.ajax({
		...
		success:function(data){
			...
			if(data.Status == "success"){
				//alert("ok");
				fn_bloglistUIUpdate(data.BlCnt);
			}else{
				alert("Error");
			}
		},
...
function fn_bloglistUIUpdate(BlCnt){
	//alert(GpaAvg+"-"+GpaCnt);
...
function fn_blogList(Bl_prdtnum, Bl_corpCd){
	...
	$.ajax({
		...
		// dataType:"json",
		async:false,
		url:g_Oto_getContextPath+'/mw/cmm/blogList.ajax',
		data:parameters ,
		success:function(data){
			$("#blog").html(data);
		},
