...
function fn_Ins(){
	...
	if("${bbs.bbsNum}" =='FNREQ') {
		if(isNull($("#cmplRequestDt").val())) {
			alert("완료 요쳥일자를 입력해 주세요.");
			$("#cmplRequestDt").focus();
			return ;
		}
		$("#cmplRequestDt").val($('#cmplRequestDt').val().replace(/-/g, ""));
	}
	<c:if test="${bbs.edtYn eq 'Y'}" >
	...
	document.frm.action = "<c:url value='/oss/nbbs/bbsReg.do'/>";
	document.frm.submit();
}
		...
		<div id="contents_area"> 
			<div id="contents">
				...
				<form name="frm" id="frm" method="post" enctype="multipart/form-data" onSubmit="return false;" >
					...
					<input type="hidden" id="htmlYn" name="htmlYn" value="${bbs.edtYn}" />
						...
						<table border="1" class="table02">
		                	...
		                	<c:if test="${bbs.bbsNum=='FNREQ'}">
		                		<tr>
		                			<th>완료 요청 일자</th>
		                			<td colpan="3">
		                				<input type="text" name="cmplRequestDt" id="cmplRequestDt" class="input_text4 center"/>
		                			</td>
		                		</tr>
		                	</c:if>
						</table>
				...
				<ul class="btn_rt01">
					...
					<li class="btn_sty01">
						<a href="<c:url value='/oss/nbbs/bbsList.do'/>?bbsNum=${notice.bbsNum}&pageIndex=${searchVO.pageIndex}&sKeyOpt=${searchVO.sKeyOpt}&sKey=${searchVO.sKey}">취소</a>
					</li>
				</ul>
