...
function fn_Search(corpNm){	
	...
	document.frm.action = "<c:url value='/oss/bisSalePdfSv.do'/>";
	document.frm.submit();
}
...
var type_config = {
    ...
    data: {
        datasets: [{
            data: [<c:forEach items="${svTypePer }" var="per">${per.rsvCntPer},</c:forEach>],
            backgroundColor: lebels_bgcolors,
        }],
        labels: [<c:forEach items="${svTypePer }" var="per">"${per.prdtNm}",</c:forEach>]
    },
    options: {
        responsive: false,
        legend: {
...
var buy_cnt_config = {
	    ...
	    data: {
	        datasets: [{
	            data: [<c:forEach items="${svBuyCntPer }" var="per">${per.rsvCntPer},</c:forEach>],
	            backgroundColor: lebels_bgcolors,
	        }],
	        labels: [<c:forEach items="${svBuyCntPer }" var="per">"${per.prdtNm}",</c:forEach>]
	    },
	    options: {
	        responsive: false,
	        legend: {
...
var cancel_rsv_config = {
	    ...
	    data: {
	        datasets: [{
	            data: [<c:forEach items="${svCancelRsvPer }" var="per">${per.rsvCntPer},</c:forEach>],
	            backgroundColor: lebels_bgcolors,
	        }],
	        labels: [<c:forEach items="${svCancelRsvPer }" var="per">"${per.prdtNm}",</c:forEach>]
	    },
	    options: {
	        responsive: false,
	        legend: {
...
window.onload = function() {	
    var svType = document.getElementById("chart-type").getContext("2d");
    var svBuyCnt = document.getElementById("chart-buy_cnt").getContext("2d");
    var svCancelRsv = document.getElementById("chart-cancel_rsv").getContext("2d");
    window.myPie = new Chart(svType, type_config);
    window.myPie = new Chart(svBuyCnt, buy_cnt_config);
    window.myPie = new Chart(svCancelRsv, cancel_rsv_config);
            ...
            <!-- change contents -->
                ...
                <div id="menu_depth3">
					<ul>
						...
						<li class="on"><a class="menu_depth3" href="/oss/bisSalePdfSv.do">제주특산/기념품</a></li>
				...
				<input type="hidden" name="sCategory" value="SV" />
			    ...
			    <div class="search-value2">
			    	...
			    	<c:set var="searchType" value="제주특산/기념품" />
