...
function fn_Search(corpNm){	
	...
	document.frm.action = "<c:url value='/oss/bisSalePdfSpf.do'/>";
	document.frm.submit();
}
...
var type_config = {
    ...
    data: {
        datasets: [{
            data: [<c:forEach items="${spfTypePer }" var="per">${per.rsvCntPer},</c:forEach>],
            backgroundColor: lebels_bgcolors,
        }],
        labels: [<c:forEach items="${spfTypePer }" var="per">"${per.prdtNm}",</c:forEach>]
    },
    options: {
        responsive: false,
        legend: {
...
var buy_cnt_config = {
	    ...
	    data: {
	        datasets: [{
	            data: [<c:forEach items="${spfBuyCntPer }" var="per">${per.rsvCntPer},</c:forEach>],
	            backgroundColor: lebels_bgcolors,
	        }],
	        labels: [<c:forEach items="${spfBuyCntPer }" var="per">"${per.prdtNm}",</c:forEach>]
	    },
	    options: {
	        responsive: false,
	        legend: {
...
var buy_time_config = {
	    ...
	    data: {
	        datasets: [{
	            data: [<c:forEach items="${spfBuyTimePer }" var="per">${per.rsvCntPer},</c:forEach>],
	            backgroundColor: lebels_bgcolors,
	        }],
	        labels: [<c:forEach items="${spfBuyTimePer }" var="per">"${per.prdtNm}",</c:forEach>]
	    },
	    options: {
	        responsive: false,
	        legend: {
...
var cancel_rsv_config = {
	    ...
	    data: {
	        datasets: [{
	            data: [<c:forEach items="${spfCancelRsvPer }" var="per">${per.rsvCntPer},</c:forEach>],
	            backgroundColor: lebels_bgcolors,
	        }],
	        labels: [<c:forEach items="${spfCancelRsvPer }" var="per">"${per.prdtNm}",</c:forEach>]
	    },
	    options: {
	        responsive: false,
	        legend: {
...
window.onload = function() {	
    var spfType = document.getElementById("chart-type").getContext("2d");
    var spfBuyCnt = document.getElementById("chart-buy_cnt").getContext("2d");
    var spfBuyTime = document.getElementById("chart-buy_time").getContext("2d");
    var spfCancelRsv = document.getElementById("chart-cancel_rsv").getContext("2d");
    window.myPie = new Chart(spfType, type_config);
    window.myPie = new Chart(spfBuyCnt, buy_cnt_config);
    window.myPie = new Chart(spfBuyTime, buy_time_config);
    window.myPie = new Chart(spfCancelRsv, cancel_rsv_config);
            ...
            <!-- change contents -->
                ...
                <div id="menu_depth3">
					<ul>
						...
						<li class="on"><a class="menu_depth3" href="/oss/bisSalePdfSpf.do">맛집</a></li>
				...
				<input type="hidden" name="sCategory" value="SPF" />
			    ...
			    <table class="table01 center tMargin">
			        ...
			        <tbody>
			            <tr>
			            	...
			            	<c:forEach var="info" items="${spfUseCnt }">
			    ...
			    <div class="chart-form">
			        ...
			        <div class="item">
			        	<h5 class="title">쿠폰 구매 개수</h5>
            			<div class="chart">
			    			<div class="pd-wrap"><canvas id="chart-buy_cnt" width="250" height="200" style="margin: 0 auto;" /></div>
			    		</div>
            			<ul class="chart-caption">
			                <c:forEach var="info" items="${spfBuyCntPer }" varStatus="state">
			                <li class="area">
			                    <div class="bg"><span class="bg${state.count }"></span></div>
			                    <div class="text">${info.prdtNm }</div>
			                    <div class="percent"><span>${info.rsvCntPer } %</span> <span>(${info.rsvCnt }건)</span></div>
			        ...
			        <div class="item">
			        	<h5 class="title">쿠폰 구매 시기</h5>
            			<div class="chart">
			    			<div class="pd-wrap"><canvas id="chart-buy_time" width="250" height="200" style="margin: 0 auto;" /></div>
			    		</div>
            			<ul class="chart-caption">
				            <c:forEach var="info" items="${spfBuyTimePer }" varStatus="state">
			                <li class="area">
			                    <div class="bg"><span class="bg${state.count }"></span></div>
			                    <div class="text">${info.prdtNm }</div>
			                    <div class="percent"><span>${info.rsvCntPer } %</span> <span>(${info.rsvCnt }건)</span></div>
			        ...
			        <div class="item">
            			...
            			<ul class="chart-caption">
			                <c:forEach var="info" items="${spfCancelRsvPer }" varStatus="state">
			                <li class="area">
			                    <div class="bg"><span class="bg${state.count }"></span></div>
			                    <div class="text">${info.prdtNm }</div>
			                    <div class="percent"><span>${info.rsvCntPer } %</span> <span>(${info.rsvCnt }건)</span></div>
