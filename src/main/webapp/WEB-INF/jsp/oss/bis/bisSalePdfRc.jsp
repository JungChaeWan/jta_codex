...
function fn_Search(corpNm){	
	...
	document.frm.action = "<c:url value='/oss/bisSalePdfRc.do'/>";
	document.frm.submit();
}
...
var type_config = {
    ...
    data: {
        datasets: [{
            data: [<c:forEach items="${rcTypePer }" var="per">${per.rsvCntPer},</c:forEach>],
            backgroundColor: lebels_bgcolors,
        }],
        labels: [<c:forEach items="${rcTypePer }" var="per">"${per.prdtNm}",</c:forEach>]
    },
    options: {
        responsive: false,
        legend: {
...
var period_config = {
	    ...
	    data: {
	        datasets: [{
	            data: [<c:forEach items="${rcPeriodPer }" var="per">${per.rsvCntPer},</c:forEach>],
	            backgroundColor: lebels_bgcolors,
	        }],
	        labels: [<c:forEach items="${rcPeriodPer }" var="per">"${per.prdtNm}",</c:forEach>]
	    },
	    options: {
	        responsive: false,
	        legend: {
...
var fuel_config = {
	    ...
	    data: {
	        datasets: [{
	            data: [<c:forEach items="${rcFuelPer }" var="per">${per.rsvCntPer},</c:forEach>],
	            backgroundColor: lebels_bgcolors,
	        }],
	        labels: [<c:forEach items="${rcFuelPer }" var="per">"${per.prdtNm}",</c:forEach>]
	    },
	    options: {
	        responsive: false,
	        legend: {
...
var cancel_rsv_config = {
	    ...
	    data: {
	        datasets: [{
	            data: [<c:forEach items="${rcCancelRsvPer }" var="per">${per.rsvCntPer},</c:forEach>],
	            backgroundColor: lebels_bgcolors,
	        }],
	        labels: [<c:forEach items="${rcCancelRsvPer }" var="per">"${per.prdtNm}",</c:forEach>]
	    },
	    options: {
	        responsive: false,
	        legend: {
...
var cancel_use_config = {
	    ...
	    data: {
	        datasets: [{
	            data: [<c:forEach items="${rcCancelUsePer }" var="per">${per.rsvCntPer},</c:forEach>],
	            backgroundColor: lebels_bgcolors,
	        }],
	        labels: [<c:forEach items="${rcCancelUsePer }" var="per">"${per.prdtNm}",</c:forEach>]
	    },
	    options: {
	        responsive: false,
	        legend: {
...
<c:forEach items="${categoryList }" var="cd">			        
  <c:if test="${empty searchVO.sType || cd.cdNum eq searchVO.sType }">
  	<c:set var="typeCntPer">rcType${cd.cdNum }Per</c:set>
var type_${cd.cdNum}_config = {
	    type: 'pie',
	    data: {
	        datasets: [{
	            data: [<c:forEach items="${requestScope[typeCntPer] }" var="per">${per.rsvCntPer},</c:forEach>],
	            backgroundColor: lebels_bgcolors,
	        }],
	        labels: [<c:forEach items="${requestScope[typeCntPer] }" var="per">"${per.prdtNm}",</c:forEach>]
	    },
	    options: {
	        responsive: false,
	        legend: {
...
window.onload = function() {	
    var rcType = document.getElementById("chart-type").getContext("2d");
    var rcPeriod = document.getElementById("chart-period").getContext("2d");
    var rcFuel = document.getElementById("chart-fuel").getContext("2d");
    var rcCancelRsv = document.getElementById("chart-cancel_rsv").getContext("2d");
    var rcCancelUse = document.getElementById("chart-cancel_use").getContext("2d");
    window.myPie = new Chart(rcType, type_config);
    window.myPie = new Chart(rcPeriod, period_config);
    window.myPie = new Chart(rcFuel, fuel_config);
    window.myPie = new Chart(rcCancelRsv, cancel_rsv_config);
    window.myPie = new Chart(rcCancelUse, cancel_use_config);
    
    <c:forEach items="${categoryList }" var="cd">			        
      <c:if test="${empty searchVO.sType || cd.cdNum eq searchVO.sType }">
    var rc${cd.cdNum} = document.getElementById("chart-${cd.cdNum}").getContext("2d");
    window.myPie = new Chart(rc${cd.cdNum}, type_${cd.cdNum}_config);
      </c:if>
    </c:forEach>
            ...
            <!-- change contents -->
                ...
                <div id="menu_depth3">
					<ul>
						...
						<li class="on"><a class="menu_depth3" href="/oss/bisSalePdfRc.do">렌터카</a></li>
				...
				<input type="hidden" name="sCategory" value="RC" />
			    ...
			    <table class="table01 center tMargin">
			        ...
			        <tbody>
			            <tr>
			            	...
			            	<c:forEach var="info" items="${rcStartCnt }">
			    ...
			    <div class="chart-form">
			        ...
			        <div class="item">
			        	<h5 class="title">연료 유형<small> (단품 미포함)</small></h5>
            			<div class="chart">
			    			<div class="pd-wrap"><canvas id="chart-fuel" width="250" height="200" style="margin: 0 auto;" /></div>
			    		</div>
            			<ul class="chart-caption">
			                <c:forEach var="info" items="${rcFuelPer }" varStatus="state">
			                <li class="area">
			                    <div class="bg"><span class="bg${state.count }"></span></div>
			                    <div class="text">${info.prdtNm }</div>
			                    <div class="percent"><span>${info.rsvCntPer } %</span> <span>(${info.rsvCnt }건)</span></div>
			        ...
			        <c:forEach items="${categoryList }" var="cd">			        
			          <c:if test="${empty searchVO.sType || cd.cdNum eq searchVO.sType }">
			        <div class="item">
			        	<h5 class="title">${cd.cdNm }<small> (단품 미포함)</small></h5>
            			<div class="chart">
			    			<div class="pd-wrap"><canvas id="chart-${cd.cdNum }" width="250" height="200" style="margin: 0 auto;" /></div>
			    		</div>
            			<ul class="chart-caption">
            			  <c:set var="typeCntPer">rcType${cd.cdNum }Per</c:set>
			                <c:forEach var="info" items="${requestScope[typeCntPer] }" varStatus="state">
			                <li class="area">
			                    <div class="bg"><span class="bg${state.count }"></span></div>
			                    <div class="text">${info.prdtNm }</div>
			                    <div class="percent"><span>${info.rsvCntPer } %</span> <span>(${info.rsvCnt }건)</span></div>
			        ...
			        <div class="item">
            			...
            			<ul class="chart-caption">
			                <c:forEach var="info" items="${rcCancelRsvPer }" varStatus="state">
			                <li class="area">
			                    <div class="bg"><span class="bg${state.count }"></span></div>
			                    <div class="text">${info.prdtNm }</div>
			                    <div class="percent"><span>${info.rsvCntPer } %</span> <span>(${info.rsvCnt }건)</span></div>
			        ...
			        <div class="item">
				        ...
				        <ul class="chart-caption">
				            <c:forEach var="info" items="${rcCancelUsePer }" varStatus="state">
			                <li class="area">
			                    <div class="bg"><span class="bg${state.count }"></span></div>
			                    <div class="text">${info.prdtNm }</div>
			                    <div class="percent"><span>${info.rsvCntPer } %</span> <span>(${info.rsvCnt }건)</span></div>
