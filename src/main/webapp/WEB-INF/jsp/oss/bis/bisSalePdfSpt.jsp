...
function fn_Search(corpNm){	
	...
	document.frm.action = "<c:url value='/oss/bisSalePdfSpt.do'/>";
	document.frm.submit();
}
...
//chart 관련
var lebels_bgcolors = ["#31c4b5", "#fa6a4f", "#e9ac21", "#4b5c6a", "#d79999", "#8784bd"];
var type_config = {
    type: 'pie',
    data: {
        datasets: [{
            data: [<c:forEach items="${sptTypePer }" var="per">${per.rsvCntPer},</c:forEach>],
            backgroundColor: lebels_bgcolors,
        }],
        labels: [<c:forEach items="${sptTypePer }" var="per">"${per.prdtNm}",</c:forEach>]
    },
    options: {
        responsive: false,
        legend: {
...
var price_config = {
	    ...
	    data: {
	        datasets: [{
	            data: [<c:forEach items="${sptPricePer }" var="per">${per.rsvCntPer},</c:forEach>],
	            backgroundColor: lebels_bgcolors,
	        }],
	        labels: [<c:forEach items="${sptPricePer }" var="per">"${per.prdtNm}",</c:forEach>]
	    },
	    options: {
	        responsive: false,
	        legend: {
...
var cancel_rsv_config = {
	    ...
	    data: {
	        datasets: [{
	            data: [<c:forEach items="${sptCancelRsvPer }" var="per">${per.rsvCntPer},</c:forEach>],
	            backgroundColor: lebels_bgcolors,
	        }],
	        labels: [<c:forEach items="${sptCancelRsvPer }" var="per">"${per.prdtNm}",</c:forEach>]
	    },
	    options: {
	        responsive: false,
	        legend: {
...
var cancel_use_config = {
	    ...
	    data: {
	        datasets: [{
	            data: [<c:forEach items="${sptCancelUsePer }" var="per">${per.rsvCntPer},</c:forEach>],
	            backgroundColor: lebels_bgcolors,
	        }],
	        labels: [<c:forEach items="${sptCancelUsePer }" var="per">"${per.prdtNm}",</c:forEach>]
	    },
	    options: {
	        responsive: false,
	        legend: {
...
window.onload = function() {	
    var sptType = document.getElementById("chart-type").getContext("2d");
    var sptPrice = document.getElementById("chart-price").getContext("2d");    
    var sptCancelRsv = document.getElementById("chart-cancel_rsv").getContext("2d");
    var sptCancelUse = document.getElementById("chart-cancel_use").getContext("2d");
    window.myPie = new Chart(sptType, type_config);
    window.myPie = new Chart(sptPrice, price_config);
    window.myPie = new Chart(sptCancelRsv, cancel_rsv_config);
    window.myPie = new Chart(sptCancelUse, cancel_use_config);
            ...
            <!-- change contents -->
                ...
                <div id="menu_depth3">
					<ul>
						...
						<li class="on"><a class="menu_depth3" href="/oss/bisSalePdfSpt.do">여행사 상품</a></li>
				...
				<input type="hidden" name="sCategory" value="SPT" />
			    ...
			    <div class="search-value2">
			    	...
			    	<c:set var="searchType" value="여행사 상품" />
			    ...
			    <div class="chart-form">
			        ...
			        <div class="item">
            			...
            			<ul class="chart-caption">
			                <c:forEach var="info" items="${sptCancelRsvPer }" varStatus="state">
			                <li class="area">
			                    <div class="bg"><span class="bg${state.count }"></span></div>
			                    <div class="text">${info.prdtNm }</div>
			                    <div class="percent"><span>${info.rsvCntPer } %</span> <span>(${info.rsvCnt }건)</span></div>
			        ...
			        <div class="item">
				        ...
				        <ul class="chart-caption">
				            <c:forEach var="info" items="${sptCancelUsePer }" varStatus="state">
			                <li class="area">
			                    <div class="bg"><span class="bg${state.count }"></span></div>
			                    <div class="text">${info.prdtNm }</div>
			                    <div class="percent"><span>${info.rsvCntPer } %</span> <span>(${info.rsvCnt }건)</span></div>
