...
function fn_Search(corpNm){	
	...
	document.frm.action = "<c:url value='/oss/bisSalePdfSpb.do'/>";
	document.frm.submit();
}
...
var buy_cnt_config = {
	    ...
	    data: {
	        datasets: [{
	            data: [<c:forEach items="${spbBuyCntPer }" var="per">${per.rsvCntPer},</c:forEach>],
	            backgroundColor: lebels_bgcolors,
	        }],
	        labels: [<c:forEach items="${spbBuyCntPer }" var="per">"${per.prdtNm}",</c:forEach>]
	    },
	    options: {
	        responsive: false,
	        legend: {
...
var buy_time_config = {
	    ...
	    data: {
	        datasets: [{
	            data: [<c:forEach items="${spbBuyTimePer }" var="per">${per.rsvCntPer},</c:forEach>],
	            backgroundColor: lebels_bgcolors,
	        }],
	        labels: [<c:forEach items="${spbBuyTimePer }" var="per">"${per.prdtNm}",</c:forEach>]
	    },
	    options: {
	        responsive: false,
	        legend: {
...
var cancel_rsv_config = {
	    ...
	    data: {
	        datasets: [{
	            data: [<c:forEach items="${spbCancelRsvPer }" var="per">${per.rsvCntPer},</c:forEach>],
	            backgroundColor: lebels_bgcolors,
	        }],
	        labels: [<c:forEach items="${spbCancelRsvPer }" var="per">"${per.prdtNm}",</c:forEach>]
	    },
	    options: {
	        responsive: false,
	        legend: {
            ...
            <!-- change contents -->
                ...
                <div id="menu_depth3">
					<ul>
						...
						<li class="on"><a class="menu_depth3" href="/oss/bisSalePdfSpb.do">카시트/유모차</a></li>
			    ...
			    <div class="search-value2">
			    	...
			    	<c:set var="searchType" value="카시트/유모차" />
			    ...
			    <div class="chart-form">			        
			        ...
			        <div class="item">
            			...
            			<ul class="chart-caption">
			                <c:forEach var="info" items="${spbCancelRsvPer }" varStatus="state">
			                <li class="area">
			                    <div class="bg"><span class="bg${state.count }"></span></div>
			                    <div class="text">${info.prdtNm }</div>
			                    <div class="percent"><span>${info.rsvCntPer } %</span> <span>(${info.rsvCnt }건)</span></div>
