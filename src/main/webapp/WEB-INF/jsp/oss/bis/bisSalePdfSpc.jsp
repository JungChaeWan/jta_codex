...
function fn_Search(corpNm){	
	...
	document.frm.action = "<c:url value='/oss/bisSalePdfSpc.do'/>";
	document.frm.submit();
}
...
var type_config = {
    ...
    data: {
        datasets: [{
            data: [<c:forEach items="${spcTypePer }" var="per">${per.rsvCntPer},</c:forEach>],
            backgroundColor: lebels_bgcolors,
        }],
        labels: [<c:forEach items="${spcTypePer }" var="per">"${per.prdtNm}",</c:forEach>]
    },
    options: {
        responsive: false,
        legend: {
...
var buy_cnt_config = {
	    ...
	    data: {
	        datasets: [{
	            data: [<c:forEach items="${spcBuyCntPer }" var="per">${per.rsvCntPer},</c:forEach>],
	            backgroundColor: lebels_bgcolors,
	        }],
	        labels: [<c:forEach items="${spcBuyCntPer }" var="per">"${per.prdtNm}",</c:forEach>]
	    },
	    options: {
	        responsive: false,
	        legend: {
...
var buy_time_config = {
	    ...
	    data: {
	        datasets: [{
	            data: [<c:forEach items="${spcBuyTimePer }" var="per">${per.rsvCntPer},</c:forEach>],
	            backgroundColor: lebels_bgcolors,
	        }],
	        labels: [<c:forEach items="${spcBuyTimePer }" var="per">"${per.prdtNm}",</c:forEach>]
	    },
	    options: {
	        responsive: false,
	        legend: {
...
var cancel_rsv_config = {
	    ...
	    data: {
	        datasets: [{
	            data: [<c:forEach items="${spcCancelRsvPer }" var="per">${per.rsvCntPer},</c:forEach>],
	            backgroundColor: lebels_bgcolors,
	        }],
	        labels: [<c:forEach items="${spcCancelRsvPer }" var="per">"${per.prdtNm}",</c:forEach>]
	    },
	    options: {
	        responsive: false,
	        legend: {
...
window.onload = function() {	
    var spcType = document.getElementById("chart-type").getContext("2d");
    var spcBuyCnt = document.getElementById("chart-buy_cnt").getContext("2d");
    var spcBuyTime = document.getElementById("chart-buy_time").getContext("2d");
    var spcCancelRsv = document.getElementById("chart-cancel_rsv").getContext("2d");
    window.myPie = new Chart(spcType, type_config);
    window.myPie = new Chart(spcBuyCnt, buy_cnt_config);
    window.myPie = new Chart(spcBuyTime, buy_time_config);
    window.myPie = new Chart(spcCancelRsv, cancel_rsv_config);
            ...
            <!-- change contents -->
                ...
                <div id="menu_depth3">
					<ul>
						...
						<li class="on"><a class="menu_depth3" href="/oss/bisSalePdfSpc.do">관광지/레저</a></li>
				...
				<input type="hidden" name="sCategory" value="SPC" />
			    ...
			    <div class="search-value2">
			    	...
			    	<c:set var="searchType" value="관광지/레저" />
			    ...
			    <table class="table01 center tMargin">
			        ...
			        <tbody>
			            <tr>
			            	...
			            	<c:forEach var="info" items="${spcUseCnt }">
			    ...
			    <div class="chart-form">
			        ...
			        <div class="item">
            			...
            			<ul class="chart-caption">
			                <c:forEach var="info" items="${spcBuyCntPer }" varStatus="state">
			                <li class="area">
			                    <div class="bg"><span class="bg${state.count }"></span></div>
			                    <div class="text">${info.prdtNm }</div>
			                    <div class="percent"><span>${info.rsvCntPer } %</span> <span>(${info.rsvCnt }건)</span></div>
			        ...
			        <div class="item">
            			...
            			<ul class="chart-caption">
				            <c:forEach var="info" items="${spcBuyTimePer }" varStatus="state">
			                <li class="area">
			                    <div class="bg"><span class="bg${state.count }"></span></div>
			                    <div class="text">${info.prdtNm }</div>
			                    <div class="percent"><span>${info.rsvCntPer } %</span> <span>(${info.rsvCnt }건)</span></div>
			        ...
			        <div class="item">
            			...
            			<ul class="chart-caption">
			                <c:forEach var="info" items="${spcCancelRsvPer }" varStatus="state">
			                <li class="area">
			                    <div class="bg"><span class="bg${state.count }"></span></div>
			                    <div class="text">${info.prdtNm }</div>
			                    <div class="percent"><span>${info.rsvCntPer } %</span> <span>(${info.rsvCnt }건)</span></div>
