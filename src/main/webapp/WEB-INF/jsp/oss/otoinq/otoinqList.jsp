...
function fn_Dtl(otoinqNum){
	...
	document.frm.action = "<c:url value='/oss/detailOtoinq.do'/>";
	document.frm.submit();
}
	               						...
	               						<tr>
	          								<th scope="row">
	          									<select name="sKeyOpt" id="sKeyOpt" style="width:100px">
	          										<option value="5" <c:if test="${searchVO.sKeyOpt == '5'}">selected="selected"</c:if>>업체이름</option>
													<option value="1" <c:if test="${searchVO.sKeyOpt == '1'}">selected="selected"</c:if>>업체아이디</option>
													<option value="6" <c:if test="${searchVO.sKeyOpt == '6'}">selected="selected"</c:if>>상품이름</option>
	                ...
	                <table width="100%" border="1" cellspacing="0" cellpadding="0" class="table01 list_tb">
						...
						<tbody>
							...
							<c:forEach var="data" items="${resultList}" varStatus="status">
								<tr style="cursor:pointer;" onclick="fn_Dtl('${data.otoinqNum}')">
									<td class="align_ct">${data.otoinqNum}</td>
									<td class="align_ct"><c:out value='${data.corpNm2}'/>(<c:out value='${data.corpId}'/>)</td>
