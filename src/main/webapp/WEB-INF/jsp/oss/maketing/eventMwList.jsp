...
function fn_InsPrmt(){
	...
	document.frm.action = "<c:url value='/oss/mwPrmtReg.do'/>";
	document.frm.submit();
}
...
function fn_snOnchange(obj, sn_old, prmtNum){
	...
	document.frm.action = "<c:url value='/oss/mwPrmtMod.do'/>";
	document.frm.submit();
...
function fn_delPrmt(printSn, prmtNum){
	...
	if(confirm("삭제 하시겠습니까?") ){
		...
		document.frm.prmtNum.value = prmtNum;
		document.frm.action = "<c:url value='/oss/mwPrmtDel.do'/>";
		document.frm.submit();
	}
}
...
function fn_Find(){
	var retVal = window.open("<c:url value='/oss/findPrmt.do'/>","findUser", "width=700, height=650, scrollbars=yes, status=no, toolbar=no;");
}
	...
	<div id="contents_wrapper">
		...
		<div id="contents_area">
			...
			<div id="contents">
				...
				<input type="hidden" id="printSn" name="printSn" />
				...
				<div id="menu_depth3">
					<ul>
			            <li><a class="menu_depth3" href="<c:url value='/oss/eventList.do'/>">프로모션</a></li>
		              	...
		              	<li class="on"><a class="menu_depth3" href="<c:url value='/oss/eventMwList.do'/>">모바일 프로모션</a></li>
	                </ul>
	            </div>
				<p class="search_list_ps title03">모바일 프로모션 관리 [총 <strong>${totalCnt}</strong>건]</p> <!--리스트 검색 건수-->
				<div class="list">
					...
					<tbody>
						...
						<c:forEach var="prmt" items="${resultList}" varStatus="status">
							<tr >
								...
								<td>
									<c:if test="${empty prmt.corpId}">
										-탐나오-
									</c:if>
									<c:if test="${!(empty prmt.corpId)}">
										<c:out value="${prmt.corpNm}"/>
									</c:if>
								</td>
								<td class="align_ct"><fmt:parseDate value="${prmt.startDt}" var="startDt"	pattern="yyyyMMdd" />
									<fmt:parseDate value="${prmt.endDt}" var="endDt" pattern="yyyyMMdd" /> <fmt:formatDate value="${startDt}" pattern="yyyy-MM-dd" />~<fmt:formatDate value="${endDt}" pattern="yyyy-MM-dd" /></td>
								<td class="align_ct">
									...
									<c:if test="${Constant.TRADE_STATUS_APPR_REJECT eq prmt.statusCd}">
										승인거절
									</c:if>
								...
								<td class="align_ct">
									<c:if test="${prmt.endEvent == 'Y'}">
										마감
									</c:if>
									<c:if test="${prmt.endEvent == 'N'}">
										진행
									</c:if>
								</td>
								<td class="align_ct">
								...
								<td class="align_ct">
									<div class="btn_sty09">
										<span><a href="javascript:fn_delPrmt(${prmt.printSn}, '${prmt.prmtNum}', 'Y')">삭제</a></span>
									</div>
								</td>
							</tr>
						</c:forEach>
					...
					<table border="1" class="table02">
						...
						<tr>
							...
							<td>
								...
								<input type="text" readonly="readonly" id="prmtNmT" name="prmtNmT" class="input_text30"/>
								<input type="hidden" id='prmtNumT' name='prmtNumT'/>
								<input type="hidden" id='printSnT' name='printSnT' value="${totalCnt+1}" />
								<%-- 순번:
								<select id='printSnT' name='printSnT'>
									<c:forEach var="cnt" begin="1" end="${totalCnt+1}">
										<option value="${cnt}" <c:if test="${cnt == totalCnt+1}">selected="selected"</c:if>>${cnt}</option>
									</c:forEach>
								</select> --%>
