...
function fn_InsPrmt(){
	...
	document.frm.action = "<c:url value='/oss/svPrmtReg.do'/>";
	document.frm.submit();
}
...
function fn_snOnchange(obj, sn_old, prmtNum) {
	...
	document.frm.action = "<c:url value='/oss/svPrmtMod.do'/>";
	document.frm.submit();
}
...
function fn_delPrmt(printSn, prmtNum) {
	if(confirm("삭제 하시겠습니까?")) {
		...
		document.frm.prmtNum.value = prmtNum;
		document.frm.action = "<c:url value='/oss/svPrmtDel.do'/>";
		document.frm.submit();
	}
}

function fn_Find() {
	window.open("<c:url value='/oss/findPrmt.do?prmtDiv=FROM'/>","findUser", "width=700, height=650, scrollbars=yes, status=no, toolbar=no;");
}
				...
				<form name="frm" method="get" onSubmit="return false;">
					...
					<div id="menu_depth3">
						<ul>
							...
							<li class="on"><a class="menu_depth3" href="<c:url value='/oss/eventSvList.do'/>">제주특산/기념품 프로모션</a></li>
						</ul>
					</div>

					<p class="search_list_ps title03">제주특산/기념품 프로모션 [총 <strong>${totalCnt}</strong>건]</p> <!--리스트 검색 건수-->
					<div class="list">
