...
<meta property="og:title" content="제주 특산 기념품 목록">
<meta property="og:url" content="https://www.tamnao.com/mw/sv/productList.do">
<meta property="og:description" content="탐나오 제주특산/기념품 목록" >
...
function fn_Search() {
	document.frm.action = "<c:url value='/mw/sv/productList.do'/>";
	document.frm.submit();
}
		...
		<div class="menu-typeB">
			...
			<nav id="scroll_menuB" class="scroll-menuB">
				<div class="scroll-area">
					<ul>
						<c:if test="${empty param.sFlag}">
							<li><a href="<c:url value='/mw/sv/productList.do?sCtgr=${searchVO.sCtgr}'/>" <c:if test="${empty param.sSubCtgr}">class="on"</c:if>>전체</a></li>
						</c:if>
					  	<c:forEach items="${subCtgrMap[searchVO.sCtgr]}" var="subCtgr">
							<li>
								<a href="<c:url value='/mw/sv/productList.do?sCtgr=${searchVO.sCtgr}&sSubCtgr=${subCtgr.cdNum}'/>"  <c:if test="${param.sSubCtgr eq subCtgr.cdNum}">class="on"</c:if>>${subCtgr.cdNm}</a>
							</li>
					  	</c:forEach>
						<c:if test="${param.sFlag == 'Brd'}">
						  	<c:forEach items="${brandPrdtList }" var="brandList" varStatus="status">
								<li>
									<a href="<c:url value='/mw/sv/productList.do?sCorpId=${brandList.corpId}&sFlag=Brd'/>" <c:if test="${param.sCorpId eq brandList.corpId}">class="on"</c:if>>${brandList.corpAliasNm}</a>
								</li>
						  	</c:forEach>
						</c:if>
					</ul>
