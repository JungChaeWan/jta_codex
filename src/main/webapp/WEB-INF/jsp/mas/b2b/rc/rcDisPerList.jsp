...
function fn_ListPrdt(){
	document.defDisPer.action = "<c:url value='/mas/b2b/rc/corpGrpDisPerList.do'/>";
...
function fn_InsDisPer(){
	...
	if($("#gubun").val() == "U"){
		document.defDisPer.action = "<c:url value='/mas/b2b/rc/updateDefDisPer.do'/>";
		document.defDisPer.submit();
	}else if($("#gubun").val() == "I"){
		document.defDisPer.action = "<c:url value='/mas/b2b/rc/insertDefDisPer.do'/>";
		document.defDisPer.submit();
	}
}
...
function fn_UdtRangeDisPerLay(disPerNum){
	...
	parameters += "&disPerGrpNum=" + $("#disPerGrpNum").val();
	
	$.ajax({
		type:"post", 
		dataType:"json",
		async:false,
		url:"<c:url value='/mas/b2b/rc/selectByDisPerInf.ajax'/>",
		data:parameters ,
...
function fn_InsRangeDisPer(){
	...
	if($("#gubun2").val() == "U"){
		...
		$.ajax({
			...
			dataType:"json",
			async:false,
			url:"<c:url value='/mas/b2b/rc/updateRangeDisPer.ajax'/>",
			data:parameters ,
			success:function(data){
				if(data.success == "N"){
					alert(data.rtnMsg);
				}else{
					alert("기간 할인율이 수정되었습니다.");
					
					document.rangeDisPer.action = "<c:url value='/mas/b2b/rc/corpGrpDisPer.do'/>";
		...
		$.ajax({
			...
			dataType:"json",
			async:false,
			url:"<c:url value='/mas/b2b/rc/insertRangeDisPer.ajax'/>",
			data:parameters ,
			success:function(data){
				if(data.success == "N"){
					alert(data.rtnMsg);
				}else{
					alert("기간 할인율이 등록되었습니다.");
...
function fn_DelRangeDisPer(disPerNum){
	if(confirm("<spring:message code='common.delete.msg'/>")){
		...
		document.rangeDisPer.action = "<c:url value='/mas/b2b/rc/deleteRangeDisPer.do'/>";
		document.rangeDisPer.submit();
	}
}
...
<div id="wrapper">
	...
	<div id="contents_wrapper">
		<div id="contents_area">
			<div id="contents">
				<h2 class="title08"><c:out value="${disPerGrpVO.disPerNm}" />&nbsp;>&nbsp;<c:out value="${prdtInf.prdtNm}"/></h2>
				<form name="disPerInf" method="post" onSubmit="return false;">
				<input type="hidden" name="disPerGrpNum" id="disPerGrpNum" value="${disPerGrpVO.disPerGrpNum}" />
                ...
                <div class="list">	
                	...
                	<div id="lay_popup" class="lay_popup lay_ct" style="display:none;"> <!--왼쪽정렬:lay_lt, 오른쪽정렬:lay_rt, 가운데정렬:lay_ct--> 
			            ...
			            <form name="defDisPer">
			            	<input type="hidden" id="prdtNum" name="prdtNum" value="${disPerInfVO.prdtNum}" />
			            	<input type="hidden" name="disPerGrpNum" value="${disPerGrpVO.disPerGrpNum}" />
                	...
                	<div id="lay_popup2" class="lay_popup lay_ct" style="display:none;"> <!--왼쪽정렬:lay_lt, 오른쪽정렬:lay_rt, 가운데정렬:lay_ct--> 
			            ...
			            <form id="RC_DISPERINFVO" name="rangeDisPer">
			            	<input type="hidden" id="rangePrdtNum" name="prdtNum" value="${disPerInfVO.prdtNum}" />
			            	<input type="hidden" id="rangeDisPerGrpNum" name="disPerGrpNum" value="${disPerGrpVO.disPerGrpNum}" />
			            	...
			            	<li>
			            		<table border="1" class="table02" >
					            	...
					            	<tr>
										...
										<td>
											<input type="text" name="viewAplStartDt" id="viewAplStartDt" class="input_text4 center" readonly="readonly" value="${disPerInfVO.viewAplStartDt}" />
					            	...
					            	<tr>
										...
										<td>
											<input type="text" name="viewAplEndDt" id="viewAplEndDt" class="input_text4 center" readonly="readonly" value="${disPerInfVO.viewAplEndDt}" />
									...
									<tr>
										<th>평일할인율<span class="font02">*</span></th>
										<td>
											<input type="text" name="wdayDisPer" id="rangeWdayDisPer" class="input_text2 center" maxlength="3" value="${disPerInfVO.wdayDisPer}" onkeydown="javascript:fn_checkNumber();" /> %
										</td>
									</tr>
									<tr>
										<th>주말할인율<span class="font02">*</span></th>
										<td>
											<input type="text" name="wkdDisPer" id="rangeWkdDisPer" class="input_text2 center" maxlength="3" value="${disPerInfVO.wkdDisPer}" onkeydown="javascript:fn_checkNumber();" /> %
										</td>
									</tr>
								</table>
			            	</li>
