...
function fn_SimpleCal(){
	...
	if(confirm('수량을 정보를 입력(수정)하시겠습니까?')==false){
		return;
	}
...
function selectColorChange(){
	if($("select[name=id2] option:selected").css("color") == "rgb(51, 51, 51)" ){
		$("select[name=id2]").css("color","black")
	}else if($("select[name=id2] option:selected").css("color") == "rgb(0, 0, 0)" ){
		$("select[name=id2]").css("color","black")
	}else{
		$("select[name=id2]").css("color","green")
	}
...
$(document).ready(function(){
    ...
    $(window).on("beforeunload", function () {
        if (isExitPageComm == true){
        	return "달력 수량에 변경된 부분이 있습니다. 적용하지 않고 페이지를 이동 하시겠습니까?";
        }
    });
	                    ...
	                    <tr>
	                        <td valign="top" width="290"><!--간편입력폼-->
	                                    ...
	                                    <td class="td_align_lt01">
	                                    	<select name="id2" onchange="fn_Cnt(this.value);" style="width:96%;">
	                                    		<c:forEach var="data" items="${prdtList}" varStatus="status">
		                                            <option value="${data.prdtNum}" <c:if test="${data.prdtNum==adPrdinf.prdtNum }">selected="selected"</c:if>
													<c:choose>
														<c:when test="${data.tradeStatus=='TS03' }">style="color:green;"</c:when>
	                            ...
	                            <h5 class="title02">간편입력기</h5>
		                            ...
		                            <table class="quick_calendar" style="width:285px;">
		                                    ...
		                                    <td class="td_align_lt01">
		                                    	...
		                                    	<div style="display:inline-block;">
	                    							...
	                    							<input type="text" id="startDtView" name="startDtView" 
	                    								...
	                    								value="<fmt:formatDate value="${startDt}" pattern="yyyy-mm-dd"/>" 
	                    								readonly="readonly"/>
													<input type="hidden" id="startDt" name="startDt" value="${calendarVO.startDt}" />
		                                    	</div>
		                                        <div style="display:inline-block;">
		                                        	&nbsp;~
													...
													<input type="hidden" id="endDt" name="endDt" value="${calendarVO.endDt}" />
		                                        </div>
		                                        --%>
		                                	</td>
		                                </tr>
		                                ...
		                                <tr>
		                                    <th>총객실 수</th>
		                                    <td class="td_align_lt01">
		                                    	<input type="text" name="totalRoomNumSmp" class="full right numFormat" title="간편입력 판매가을 입력하세요." value="${calendarVO.totalRoomNumSmp}"/>
		                                    </td>
		                                </tr>
	                           ...
	                           <!--  </form:form> -->
	                            ...
	                            <div>
									<p><span style="background-color:#0760b2;color:#fff;">총</span><span>:총객실 수</span></p>
									<p><span style="background-color:#fa7415;color:#fff;">예</span>:예약객실 수<spbna</p>
								</div>
	                                ...
	                                <tr>
	                                    ...
	                                    <c:forEach var="data" items="${calList}" varStatus="status">
	                                    	...
	                                    	<td <c:if test="${data.ddlYn == 'Y' || totalRoomNum <= useRoomNum}">style="background-color: ${ddlColor}"</c:if> >
		                                    	...
		                                    	<div>
		                                        	<p class="calendar_price"><span>총</span> <input type="text" id="totalRoomNum" name="totalRoomNum" class="right numFormat" title="판매가을 입력하세요." value="${data.totalRoomNum}" <c:if test="${data.ddlYn == 'Y' || totalRoomNum <= useRoomNum}">style="background-color: ${ddlColor}"</c:if> onchange="fn_SetExitParm();" /></p>
		                                        	<p class="calendar_ipgum"><span>예</span> <input type="text" id="useRoomNum" name="useRoomNum" class="right" title="정상가을 입력하세요." value="${data.useRoomNum}" readonly style="background-color: #DCDCDC" /></p>
		                                            <p class="calendar_gubun">
		                                            	<select name="ddlYn" id="ddlYn" style="width:100%;<c:if test="${data.ddlYn == 'Y' || totalRoomNum <= useRoomNum}">background-color: ${ddlColor}</c:if>" onchange="fn_SetExitParm();">
