...
function fn_DownloadFile(Id){
	frmFileDown.location = "<c:url value='/mas/bbs/bbsFileDown.do?bbsNum=${notice.bbsNum}" + "&noticeNum=${notice.noticeNum}" + "&fileNum=" + Id +"'/>";
}
...
function fn_Udt(){
	...
	document.frm.action = "<c:url value='/mas/bbs/bbsModView.do'/>";
	document.frm.submit();
}
...
function fn_Del(){
	...
	if(confirm("게시글은 물론 관련 정보들도 같이 삭제됩니다.\n게시글을 삭제 하시겠습니까?")){
		...
		document.frm.action = "<c:url value='/mas/bbs/bbsDel.do'/>";
		document.frm.submit();
	}
}
...
function fn_cmtInsert(){
	...
	document.frm.action = "<c:url value='/mas/bbs/bbsRegCmt.do'/>";
	document.frm.submit();
}
...
function fn_cmtDel(cmtSn){
	if(confirm("댓글을 삭제 하시겠습니까?")){
		...
		document.frm.action = "<c:url value='/mas/bbs/bbsDelCmt.do'/>";
		document.frm.submit();
	}
}
...
function fn_cmtMod(cmtSn){
	...
	document.frm.action = "<c:url value='/mas/bbs/bbsModCmt.do'/>";
	document.frm.submit();
}
		...
		<div id="contents_area"> 
			...
			<div class="register_area">
				...
				<ul class="btn_rt01">
					...
					<li class="btn_sty01">
						<a href="<c:url value='/mas/bbs/bbsList.do'/>?bbsNum=${notice.bbsNum}&pageIndex=${searchVO.pageIndex}&sKeyOpt=${searchVO.sKeyOpt}&sKey=${searchVO.sKey}">목록</a>
					</li>
				</ul>
						...
						<c:forEach var="cmt" items="${cmtList}" varStatus="status">
							<tr>
								<td colspan="4">
									...
									<c:if test="${isLogin=='Y' && cmt.regId == userInfo.userId}">
