		...
		<table border="1" class="table01">
			...
			<c:forEach var="product" items="${resultList}"	varStatus="status">
	         <tr>
	         	...
	         	<td class="align_ct prmtProductNm">${product.prdtNm}(<fmt:parseDate value="${product.saleStartDt}" var="saleStartDt"	pattern="yyyyMMdd" />
					<fmt:parseDate value="${product.saleEndDt}" var="saleEndDt" pattern="yyyyMMdd" /><fmt:formatDate value="${saleStartDt}" pattern="yyyy-MM-dd" />~<fmt:formatDate value="${saleEndDt}" pattern="yyyy-MM-dd" />)</td>
