...
function fn_Search(){
	document.frm.action = "<c:url value='/mas/anls02Dtl.do'/>";
	document.frm.submit();
}


function fn_AnlsList(){
...
function fn_ExcelDown(){
	...
	frmFileDown.location = "<c:url value='/mas/anlsDtlExcelDown1.do?"+ parameters +"'/>";
}
			...
			<div id="contents">
				<div id="menu_depth3">
					<div class="btn_rt01">
						...
						<div class="btn_sty01"><a href="javascript:fn_AnlsList();">뒤로</a></div>
					</div>
				</div>
				<div id="menu_depth3">
					<ul>
                ...
                <div>
                    <select id="sFromYear" name="sFromYear" style="width:60px;" onchange="fn_Search();">
                    	<c:forEach begin="2014" end="${nowYear}" step="1" var="vYear">
                    ...
                    <select id="sFromMonth" name="sFromMonth" style="width:60px;" onchange="fn_Search();">
                    	<%-- <c:if test="${vYear eq searchVO.sFromYear}">
	                    	<c:forEach begin="1" end="${nowMonth}" step="1" var="vMonth">
					...
					<tbody>
						<c:forEach items="${resultList}" var="anls" varStatus="status">
							<tr>
								...
								<td class="align_rt"><fmt:formatNumber>${anls.cancelAmt}</fmt:formatNumber>원</td>
