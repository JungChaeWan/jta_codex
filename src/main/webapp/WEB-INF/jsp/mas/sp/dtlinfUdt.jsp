...
function fn_Udt() {
	...
	document.frm.action = "<c:url value='/mas/sp/dtlinfUdt.do'/>";
	document.frm.submit();
}
		...
		<div id="contents_area">
			<div id="contents">
	            ...
	            <form:form commandName="frm" name="frm" method="post">
	            	...
	            	<input type="hidden" name="spDtlinfNum"  value="${SP_DTLINFVO.spDtlinfNum }"/>
					...
					<div id="contents">
						<div class="register_area">
							<h4 class="title02">선택 사항 / 가격 안내 수정</h4>
								...
								<tr>
									...
									<td colspan="3">
										<select id=dtlinfType name="dtlinfType" onchange="fn_ChangeType(this.value);">
											...
											<option value="B" <c:if test="${SP_DTLINFVO.dtlinfType == 'B'}">selected="selected"</c:if>>가격안내</option>
										</select>
									...
									<td colspan="3">
										<select name="printSn" id="printSn">
											<c:forEach var="i" begin="1" end="${maxPos}">
												<option value="${i}" <c:if test="${i==SP_DTLINFVO.printSn}">selected="selected"</c:if>>${i}</option>
											</c:forEach>
										</select>
								...
								<tr>
									...
									<td colspan="3">
										<c:forEach var="data" items="${dtlInfItemList}" varStatus="status">
											...
											<span id="SPAN_itemDisAmt${status.count}">&nbsp;할인가격: <input type="text" name="itemDisAmt${status.count}" id="itemDisAmt${status.count}"  class="input_text10" maxlength="10" value="${data.itemDisAmt}"/></span>
											<span id="SPAN_itemEtc${status.count}">&nbsp;비고:<textarea name="itemEtc${status.count}" id="itemEtc${status.count}" cols="10" rows="2" style="width: 300px;">${data.itemEtc}</textarea></span>
											<%-- <span id="SPAN_itemEtc${status.count}">&nbsp;비고: <input type="text" name="itemEtc${status.count}" id="itemEtc${status.count}"  class="input_text10" maxlength="60" value="${data.itemEtc}"/></span> --%>
											<span><a href="javascript:fn_DelItem(${status.count})">[지우기]</a></span>
											</p>
										</c:forEach>
										<c:forEach var="i" begin="${fn:length(dtlInfItemList)+1}" end="8" step="1">
											<p>
											...
											<span id="SPAN_itemDisAmt${i}">&nbsp;할인가격: <input type="text" name="itemDisAmt${i}" id="itemDisAmt${i}"  class="input_text10" maxlength="10" value=""/></span>
