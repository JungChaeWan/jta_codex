...
<meta charset="UTF-8">
	...
	function fn_CancelRsvReq(){
		if(confirm("취소 하시겠습니까?")){
			...
			var parameters = "rsvNum=${resultVO.rsvNum}&adRsvNum=${resultVO.adRsvNum}&payDiv=${resultVO.payDiv}&cmssAmt=" + $("#cmssAmt").val()+"&cancelInf=" + $("#cancelInf").val();
			$.ajax({
				type:"post", 
				dataType:"json",
				async:false,
				url:"<c:url value='/mas/ad/cancelRsv.ajax'/>",
				data:parameters ,
				success:function(data){
					if(data.cancelDiv == "1"){
						if(data.success == "Y"){
							alert("자동취소가 정상적으로 처리 되었습니다.");
							document.frm.action = "<c:url value='/mw/mas/ad/detailRsv.do'/>";
							document.frm.submit();
						}else{
	 ...
	 */
	function fn_List(){
		document.frm.action = "<c:url value='/mw/mas/ad/rsvList.do'/>";
		document.frm.submit();
	}

	function fn_RsvIdt(){
		if(confirm("예약확인처리하시겠습니까?")){
			var parameters = "prdtRsvNum=${resultVO.adRsvNum}";
			$.ajax({
				type:"post", 
				dataType:"json",
				async:false,
		...
		<main id="main">
			<section class="contents-wrap">
			    <form name="frm" method="post">
					...
					<input type="hidden" name="adRsvNum" id="adRsvNum" value="${resultVO.adRsvNum}" />
