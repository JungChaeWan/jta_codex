...
function fn_CancelRsvReq(){
	...
	if(confirm("취소 수수료금액이 " + cmssAmt + "원이 맞습니까?\n취소진행을 원하시면 확인을 눌러주십시오.")){
		...
		parameters["adRsvNum"] = "${resultVO.adRsvNum}";
				...
				<table border="1" class="table02">
					...
					<tr>
						...
						<td>
							...
							<fmt:formatDate value="${fromDttm}" pattern="yyyy-MM-dd"/>부터 <c:out value="${resultVO.useNight}" />박
						</td>
						<th scope="row">사용인원</th>
						<td>
							성인: <c:out value="${resultVO.adultNum}" />명,
							소인: <c:out value="${resultVO.juniorNum}" />명,
							유아: <c:out value="${resultVO.childNum}" />명
						</td>
					</tr>
					<tr>
