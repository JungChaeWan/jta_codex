...
<meta charset="UTF-8">
	...
	function fn_CancelRsvReq(){
		if(confirm("취소 하시겠습니까?")){
			...
			var parameters = "rsvNum=${resultVO.rsvNum}&spRsvNum=${resultVO.spRsvNum}&payDiv=${resultVO.payDiv}&cmssAmt=" + $("#cmssAmt").val()+"&cancelInf=" + $("#cancelInf").val();
			$.ajax({
				type:"post", 
				dataType:"json",
				async:false,
	...
	<div id="wrap">
		...
		<main id="main">
			<section class="contents-wrap">
                ...
                <c:if test="${resultVO.prdtDiv eq Constant.SP_PRDT_DIV_TOUR}">
                <!--패키지 이용내역-->
				<div class="int-memoWrap">
					<table class="table-row">
						<colgroup>
							<col style="width: 32%">
							<col>
								...
								<td>
									...
									<ul class="list-style1">
										<c:forEach items="${spRsvhistList}" var="resultList" varStatus="status">
											<li>
												<p class="date">${resultList.regDttm}</p> 
												<p class="memo"><c:out value="${resultList.usehist}" /></p>
											</li>
										</c:forEach>
									</ul>
								</td>
							...
							<tr>
								...
								<td>
									<form name="spRsvhist" id="spRsvhist" method="post">
										<input type="hidden" name="spRsvNum" id="spRsvNum" value="${resultVO.spRsvNum}" />
										<textarea name="usehist" id="usehist"  rows="5" class="width100" placeholder="내용을 입력해주세요"></textarea>
										<button type="submit" class="btn width100" onClick="fn_saveUsehist(); return false;">등록</button>
									</form>
								</td>
							</tr>
							<tr>
								...
								<td>
									<form name="admMemoForm"  id="admMemoForm" method="post">
										<input type="hidden" name="spRsvNum" id="spRsvNum" value="${resultVO.spRsvNum}" />
										<textarea name="admMemo" id="admMemo" rows="5" class="width100" placeholder="내용을 입력해주세요">${resultVO.admMemo}</textarea>
										<button type="submit" class="btn width100" onclick="fn_updateAdmMemo(); return false;">저장</button>
									</form>
								</td>
							</tr>
						</tbody>
