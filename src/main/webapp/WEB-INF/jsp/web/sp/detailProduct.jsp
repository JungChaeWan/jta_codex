...
<link rel="alternate" media="only screen and (max-width: 640px)" href="https://www.tamnao.com/mw/sp/detailPrdt.do?prdtNum=${prdtInfo.prdtNum}">
...
<script type="application/ld+json">
	{
		...
		"itemListElement": [
			...
			},
			{
				...
				"position": 2,
				"item": {
					"@id": "https://www.tamnao.com/web/sp/detailPrdt.do?prdtNum=${prdtInfo.prdtNum}",
					...
					<div class="Fasten">
										...
										<c:if test="${prdtInfo.linkPrdtYn eq Constant.FLAG_N}">
												...
												<ul class="prInfo">
													...
													<li class="option">
														...
														<c:if test="${prdtInfo.prdtDiv eq Constant.SP_PRDT_DIV_TOUR}">
															...
															<article class="opCal" >
																<a class="opCal-title" id="opCal-title-opt">${idx}. 날짜를 선택하세요<span></span></a>
														...
														<c:if test="${fn:length(addOptList) > 0 }">
															<div class="comm-select comm-select3">
																<a class="select-button" id="select-button-opt3" title="추가옵션 선택">${idx + 1}. 추가옵션을 선택하세요</a>
											...
											<div class="purchasing-info">
												<div class="total-area">
													...
													<div class="tybe-A">
														...
														<%--<c:if test="${(not empty prdtInfo.useAbleTm) and (prdtInfo.useAbleTm > 0)}">
															<p class="ticket-info">본 상품은 ${prdtInfo.useAbleTm}시간 이후부터 사용가능합니다</p>
														</c:if>--%>
													</div>
													<!-- //기본형 -->
												</div>
												...
												<div class="purchase-btn-group">
													...
													<button type="button" id="cartBtn" class="comm-btn gray width40" onclick="fn_SpAddCart(); return false;">장바구니</button>
													</c:if>
													<button type="button" class="comm-btn red width60" onclick="fn_SpBuy();">바로구매</button>
												</div>
												</c:if>
						...
						<div id="tabs" class="nav-tabs2">
							<ul class="menuList">
								...
								${product.corpId }
								<c:if test="${prdtInfo.corpId ne 'C160000276'}">
								...
								<div class="type-bodyA">
									...
									<c:if test="${prdtInfo.lsLinkYn ne 'Y'}">
											...
											<div class="detail-photo old">
												<img src="${dtlImg[0].savePath}${dtlImg[0].saveFileNm}" loading="lazy" class="full" alt="상품설명">
											</div>
											</c:if>
									</c:if>
									<c:if test="${prdtInfo.lsLinkYn eq 'Y'}">
									...
									<div class="detail-photo old">
										<c:forEach var="dtlImg" items="${dtlImg}" varStatus="status">
											<c:if test="${status.index != 0}">
												<img src="${dtlImg.savePath}${dtlImg.saveFileNm}" loading="lazy" class="full" alt="상품설명">
											</c:if>
										</c:forEach>
									</div>
									</c:if>
									...
									<c:if test="${prdtInfo.prdtDiv ne Constant.SP_PRDT_DIV_FREE}">
										<c:if test="${!((fn:substring(prdtInfo.ctgr, 0,2) eq 'C1') or (fn:substring(prdtInfo.ctgr, 0,2) eq 'C4'))}">
											...
											<dl class="typeA">
												<dt>이용안내</dt>
												<dd>
													<c:if test="${prdtInfo.exprDaynumYn eq Constant.FLAG_Y }">
														<p>유효기간 : 구매일로 부터 <strong class="comm-color1">${prdtInfo.exprDaynum}</strong>일 까지 사용 가능합니다.</p>
													</c:if>
													<c:if test="${prdtInfo.exprDaynumYn eq Constant.FLAG_N }">
											...
											<c:if test="${prdtInfo.prdtNum ne 'SP00000342'}">
												<dl class="typeA">
													<dt>이용제한 안내</dt>
													<dd>
														<p>ㆍ본 상품은 렌터카, 자가운전자, 도보여행객 등 개별여행객만 사용이 가능합니다.<br>&nbsp;&nbsp;&nbsp;(일반 정기투어, 단체관광, 택시관광 등 기사를 동반한 여행인 경우 사용하실 수 없습니다.)</p>
														<p>ㆍ타 할인(국가유공자, 장애인, 경로, 다른 할인 쿠폰 등)과 중복할인 불가합니다.</p>
													</dd>
												</dl>
											</c:if>
										</c:if>
												...
												<div class="float-content distributor">
													<div class="l-area icon">
														<c:if test="${prdtInfo.superbCorpYn eq 'Y'}">
															<img class="excellence" src="/images/web/icon/excellence_02.jpg" loading="lazy" alt="우수관광사업체">
														</c:if>
														<c:if test="${empty prdtInfo.adtmImg}">
															<img src="/images/web/comm/no_image.jpg" loading="lazy" alt="ci">
														</c:if>
													...
													<div class="r-area text">
														...
														<p><c:out value="${prdtInfo.adtmSimpleExp}"/><!-- 주소 : 제주특별자치도 제주시 용담로 121 --></p>
														<p>연락처 : <c:out value="${prdtInfo.rsvTelNum}"/></p>
                                                        <c:if test="${not empty prdtInfo.adtmUrl && !fn:contains(prdtInfo.adtmUrl,'tamnao.com')}">
                                                            <p>홈페이지 : <a href="${prdtInfo.adtmUrl}" target="_blank" rel="noreferrer nofollow noopener">${prdtInfo.adtmUrl}</a></p>
                                                        </c:if>
													</div>
												</div>
											</dd>
						...
						<%--<c:if test="${prdtInfo.prdtDiv ne Constant.SP_PRDT_DIV_FREE}">
							<c:if test="${((prdtInfo.prdtDiv ne Constant.SP_PRDT_DIV_TOUR) or ((prdtInfo.prdtDiv eq Constant.SP_PRDT_DIV_TOUR) and (fn:length(otherProductList) > 1)))}">
								<div class="recommend-wrap">
									<section class="recommend-product">
										<h2 class="sec-caption">주변 숙소</h2>
										<div class="recommend-group">
											<div class="title-side-area">
												<div class="l-area">
													<c:if test="${Constant.SP_PRDT_DIV_TOUR eq prdtInfo.prdtDiv}">
														<h3 class="title-type4">추천상품</h3>
													</c:if>
													&lt;%&ndash;<c:if test="${fn:substring(prdtInfo.ctgr, 0, 2) eq 'C2'}">
														<h3 class="title-type4">주변 관광지</h3>
													</c:if>&ndash;%&gt;
													<c:if test="${(fn:length(otherProductList) > 1) and (fn:substring(prdtInfo.ctgr, 0, 2) ne 'C2') and (Constant.SP_PRDT_DIV_COUP eq prdtInfo.prdtDiv)}">
														<h3 class="title-type4">주변 숙소</h3>
													</c:if>
												</div>
												<div class="r-area text">
													<c:if test="${Constant.SP_PRDT_DIV_TOUR eq prdtInfo.prdtDiv}">
														<a href='<c:url value="/web/sp/corpPrdt.do?sCorpId=${prdtInfo.corpId}&sCtgr=${prdtInfo.ctgr}" />'>${prdtInfo.shopNm} 전체보기</a>
													</c:if>
												</div>
											</div>
											...
											<div class="promotion-content">
												<div class="swiper-container">
													<ul class="swiper-wrapper">
														&lt;%&ndash;<c:if test="${((rdtInfo.prdtDiv eq Constant.SP_PRDT_DIV_TOUR) and (fn:length(otherProductList) > 1)) or (fn:substring(prdtInfo.ctgr, 0, 2) eq 'C2')}">
															<c:forEach var="product" items="${otherProductList}">
																<c:if test="${((prdtInfo.prdtDiv eq Constant.SP_PRDT_DIV_TOUR) or (fn:substring(prdtInfo.ctgr, 0, 2) eq 'C2')) and (product.prdtNum ne prdtInfo.prdtNum)}">
														...
														<c:if test="${prdtInfo.prdtDiv ne Constant.SP_PRDT_DIV_TOUR}">
															<c:forEach var="product" items="${otherAdList}">
																<li class="swiper-slide">
																	<div class="photo">
																		<a href="<c:url value='/web/ad/detailPrdt.do?sPrdtNum=${product.prdtNum}&sAdultCnt=1&sChildCnt=0&sBabyCnt=0&sSearchYn=N'/>">
																			<img src="${product.savePath}thumb/${product.saveFileNm}" class="product" alt="product">
																		</a>
																	</div>
																	<div class="text">
