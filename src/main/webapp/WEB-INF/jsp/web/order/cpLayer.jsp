                    ...
                    <tr class="cpListTr <c:if test="${(empty cpInfo.useYn) and (usableYn eq 'N')}">hide</c:if>">
                        <td>
                            <c:if test="${(empty cpInfo.useYn) and (usableYn eq 'Y')}">
                                <button class="comm-btn sm black" id="btnCoupon${status.index}" onclick="javascript:fn_couponDownload('${cpInfo.cpId}', ${status.index});">받기</button>
                            </c:if>
                            <input type="radio" name="cpChk" id="cpChk${status.index}" <c:if test="${(empty cpInfo.useYn) or (usableYn eq 'N')}">class="hide"</c:if>>
                            <c:if test="${usableYn eq 'N'}">
                                <span class="text-red">사용 마감</span>
                            </c:if>
                        </td>
                        <td>
                        ...
                        <td>
                            ...
                            <c:if test="${fn:contains(cpTypes, cpInfo.disDiv)}">
                                ...
                                <c:if test="${cpInfo.buyMiniAmt > 0}">
                                    <br><span class="text-red">(<fmt:formatNumber>${cpInfo.buyMiniAmt}</fmt:formatNumber>원 이상)</span>
                                </c:if>
                            </c:if>
                            <%-- AS-IS 할인율적용
                            <c:if test="${cpInfo.disDiv eq Constant.CP_DIS_DIV_RATE }">
                            ...
                            <c:choose>
                            	...
                            	<c:when test="${(cpInfo.disDiv eq Constant.CP_DIS_DIV_RATE) and (cpInfo.limitAmt > 0)}">
                            		<c:choose>
                            			<c:when test="${(saleAmt * (cpInfo.disPct / 100)) >=  cpInfo.limitAmt}">
                            				<fmt:formatNumber>${cpInfo.limitAmt}</fmt:formatNumber>원[${cpInfo.disPct}%]
                            			</c:when>
                            			<c:otherwise>
                            				<fmt:formatNumber maxFractionDigits="0">${saleAmt * (cpInfo.disPct / 100)}</fmt:formatNumber>원[${cpInfo.disPct}%]
                            			</c:otherwise>
                            		...
                            		<c:choose>
                            			...
                            			<c:when test="${(cpInfo.buyMiniAmt eq 0) and (cpInfo.limitAmt > 0)}">
                            				<br><span class="text-red">(<fmt:formatNumber>${cpInfo.limitAmt}</fmt:formatNumber>원 이하)</span>
                            			</c:when>
                            			<c:when test="${(cpInfo.buyMiniAmt > 0) and (cpInfo.limitAmt > 0)}">
                            				<br>
                            				<%--<span class="text-red">(<fmt:formatNumber>${cpInfo.buyMiniAmt}</fmt:formatNumber>원 이상</span>--%>
                            				<span class="text-red">(최대 <fmt:formatNumber>${cpInfo.limitAmt}</fmt:formatNumber>원)</span>
                            			</c:when>
                            		</c:choose>
                            	</c:when>
                            </c:choose>
                        ...
                        <td>
                            ...
                            <c:if test="${usableYn eq 'N'}"><del></c:if>
                            <span> ~ <fmt:formatDate value="${endDt}" pattern="yyyy-MM-dd" /></span>
                            <c:if test="${usableYn eq 'N'}"></del></c:if>
                        </td>
                    </tr>
                </c:if>
...
<p class="comm-button1">
	<a href="javascript:void(0)" class="color1" onclick="fn_CpLayerOk('${selectSn}');">선택완료</a>
	<a href="javascript:void(0)" onclick="fn_CpLayerClose();">닫기</a>
</p>
